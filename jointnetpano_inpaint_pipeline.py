...
class StableDiffusionJointNetPanoInpaintPipeline(DiffusionPipeline, TextualInversionLoaderMixin, LoraLoaderMixin, FromSingleFileMixin):
    ...
    def __call__(
        ...
        view_stride: int = 8,
        view_window_size: Optional[int] = None,
        ...
        # 8. Define panorama grid and initialize views for synthesis.
        ...
        view_window_size = view_window_size if view_window_size is not None else min(latent_height, latent_width)
        views = self.get_views(height, width, window_size=view_window_size, stride=view_stride, circular_padding=circular_padding, add_full_image=include_full_pano)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # concat latents and masked latents for cropping
                latents_packed = torch.cat([latents, mask_image[:1], masked_image_latents[:1], mask_joint[:1], masked_joint_latents[:1]], dim=1)
                ...
                for j, batch_view in enumerate(views_batch):
                    ...
                    for h_start, h_end, w_start, w_end in batch_view:
                        ...
                        if circular_padding and w_end > latents.shape[3]:
                            ...
                            latent_view = torch.cat(
                                (
                                    latents_packed[:, :, h_start:h_end, w_start:],
                                    latents_packed[:, :, h_start:h_end, : w_end - latents.shape[3]],
                                ),
                                axis=-1,
                            )
                        else:
                            latent_view = latents_packed[:, :, h_start:h_end, w_start:w_end]
                    ...
                    # unpack
                    latent_model_input, mask_image_views, masked_image_latents_views, mask_joint_views, masked_joint_latents_views = \
                        latent_model_input.split([num_channels_latents, 1, num_channels_latents//2, 1, num_channels_latents//2], dim=1)
                    latents_for_view = latents_for_view[:, :num_channels_latents]
                    ...
                    noise_pred = denoise(
                        ...
                        latent_model_input, mask_image_views, masked_image_latents_views, mask_joint_views, masked_joint_latents_views,
                        t, prompt_embeds_input, cross_attention_kwargs, concat_mask=has_masked_latent)
                    ...
                    if lpips_weight > 0.0 and not full_image_batch:
                        ...
                        if lpips_skip_overlap and not lpips_anchor_batch:
                            non_overlapped = [i for i, (y_start,_,x_start,_) in enumerate(batch_view) if x_start % view_window_size == 0 and y_start % view_window_size == 0]
                            latents_for_view_selected = latents_for_view[non_overlapped]
                            noise_pred_selected = noise_pred[non_overlapped]
                        elif not lpips_skip_overlap and not lpips_anchor_batch:
                    ...
                    for latents_view_denoised, (h_start, h_end, w_start, w_end) in zip(
                        ...
                        if full_image_batch:
                            ...
                            increment_pos_x = torch.arange(view_window_size, device=latents_view_denoised.device, dtype=latents_view_denoised.dtype) + 0.5
                            increment_pos_x = increment_pos_x.unsqueeze(0).repeat(view_window_size, 1)  # HW
                            increment_pos_y = torch.arange(view_window_size, device=latents_view_denoised.device, dtype=latents_view_denoised.dtype) + 0.5
                            increment_pos_y = increment_pos_y.unsqueeze(1).repeat(1, view_window_size)  # HW
                            if boundary_type['type'] == 'constant':
                                ...
                                boundary_x_on = boundary_type['x']
                                boundary_y_on = boundary_type['y']
                                increment_weight_x = torch.ones_like(increment_pos_x) if not boundary_x_on else \
                                    ((view_window_size/2 - (increment_pos_x - view_window_size/2).abs()).clamp(0, boundary_size) / boundary_size)
                                increment_weight_y = torch.ones_like(increment_pos_y) if not boundary_y_on else \
                                    ((view_window_size/2 - (increment_pos_y - view_window_size/2).abs()).clamp(0, boundary_size) / boundary_size)
                                increment_weight = torch.min(increment_weight_x, increment_weight_y)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--prompt', required=False, type=str, default='A film photo of a beachside street under the sunset')
    ...
    parser.add_argument('--preset', type=str, default='rgb2j',
                        choices=['custom', 'rgb2j', 'j2rgb', 'expand'])
    ...
    match args.preset:
        ...
        case 'expand':  # partial rgb to rgbj
            image_input = read_image(args.image)
            input_height, input_width, _ = image_input.shape
            assert input_height == args.H
            w_start = (args.W - input_width) // 2
            # padding with relfected margin
            pano_input = np.zeros((args.H, args.W, 3), dtype='float32')
            pano_input[:, (w_start-input_width):w_start, :] = image_input[:, ::-1, :]
            pano_input[:, w_start:(w_start+input_width), :] = image_input
            pano_input[:, (w_start+input_width):(w_start+input_width*2), :] = image_input[:, ::-1, :]
            image_input = pano_input
            # construct mask
            image_mask = np.ones_like((args.H, args.W, 1), dtype='float32')
            image_mask[:, w_start:(w_start+input_width)] = 0
            joint_input = np.zeros_like(image_input)
            joint_mask = get_dummy_mask(image_input, 1)
        case 'custom':
            image_input = read_image(args.image)
    ...
    pipe_output = pipe(args.prompt, negative_prompt=args.negative_prompt, height=args.H, width=args.W,
                       ...
                       boundary_type={'type':'linear', 'size':8, 'x': True, 'y': False}, lpips_weight={'init':0.0,'decay':0.95,'skip_overlap':False})
