...
JOINTNET_SINGLE = os.environ.get('JOINTNET_SINGLE', '0') == '1'
JOINTNET_TRAIN_SINGLE = os.environ.get('JOINTNET_TRAIN_SINGLE', '0') == '1'
...
def denoiseV2(jointnet, unet, latent_model_input, t, prompt_embeds, cross_attention_kwargs):
    ...
    break_prob = 0.5
    break_link = JOINTNET_TRAIN_SINGLE and np.random.random() < break_prob
    ...
    rgb_noise_pred = unet.forward_up(
        down_block_additional_residuals=jnt_down_block_res_samples if not break_link else None,
        mid_block_additional_residual=jnt_mid_block_res_sample if not break_link else None,
        **rgb_context
    )
    jnt_noise_pred = jointnet.forward_up(
        down_block_additional_residuals=rgb_down_block_res_samples if not break_link else None,
        mid_block_additional_residual=rgb_mid_block_res_sample if not break_link else None,
        **jnt_context
    )
...
def denoiseV2_single(jointnet, unet, latent_model_input, t, prompt_embeds, cross_attention_kwargs):
    noise_pred = jointnet(
...
denoise = denoiseV2_single if JOINTNET_SINGLE else denoiseV2


class StableDiffusionJointNetPipelineOutput(BaseOutput):
    images: Union[List[PIL.Image.Image], np.ndarray]
    nsfw_content_detected: Optional[List[bool]]
    joint_outputs: Union[List[PIL.Image.Image], np.ndarray]
...
class StableDiffusionJointNetPipeline(DiffusionPipeline, TextualInversionLoaderMixin, LoraLoaderMixin, FromSingleFileMixin):
    ...
    def __call__(
        ...
        num_channels_latents = self.unet.config.in_channels * (1 if JOINTNET_SINGLE else 2)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                noise_pred = denoise(self.jointnet, self.unet, latent_model_input, t, prompt_embeds, cross_attention_kwargs)
        ...
        if JOINTNET_SINGLE:
            image_latents = joint_latents = latents
        else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--prompt', required=False, type=str, default='A living room with a picture of mountain on the wall')
    ...
    if args.platform == 'mps':
        pipe = StableDiffusionJointNetPipeline.from_pretrained_w_jointnet(args.base_model, args.model, scheduler=scheduler)
        pipe = pipe.to('mps')
        pipe.enable_attention_slicing()
    else:
        pipe = StableDiffusionJointNetPipeline.from_pretrained_w_jointnet(args.base_model, args.model, scheduler=scheduler, torch_dtype=torch.float16)
        pipe = pipe.to("cuda")
